...
class NWMSV_CopyColorspaceSetting(Operator, NWBase):
    bl_idname = "node.nw_copy_colorspace_setting"
    bl_label = "Copy Node Color Space Setting"
    bl_options = {'REGISTER', 'UNDO'}
    @classmethod
    def poll(cls, context):
        valid = False
    ...
    def execute(self, context):
        ...
        # Check if active node is texture node
        if node_active.type != 'TEX_IMAGE':
            self.report({'ERROR'}, "Active node is not a texture node")
            return {'CANCELLED'}

        # Filter out non-texture nodes from selection
        texture_nodes = [n for n in node_selected if n.type == 'TEX_IMAGE']

        # Check if there are at least 2 texture nodes selected
        if len(texture_nodes) < 2:
            self.report({'ERROR'}, "At least 2 texture nodes must be selected")
            return {'CANCELLED'}

        # Copy color space setting from active node
        active_color_space = node_active.image.colorspace_settings.name

        for node in texture_nodes:
            node.image.colorspace_settings.name = active_color_space

        self.report({'INFO'}, "Color space setting copied from active node to selected texture nodes")
        return {'FINISHED'}
